...
Semantic::Semantic(AbstractSyntaxNode *root) : ir_indent_(4) {
    root_ = root;
    current_ = root_;
}
...
Token Semantic::analyse_program() {
...
// 分析函数
Token Semantic::analyse_function(const int &pos) {
    current_ = child(pos);
...
// 解析语句
Token Semantic::analyse_statement(const int &pos, const Symbol &function_symbol) {
    current_ = child(pos);
...
// 解析 if 语句
Token Semantic::analyse_if_statement(const int &pos, const Symbol &function_symbol) {
    current_ = child(pos);
    Token result;
...
// 解析 while 语句
Token Semantic::analyse_while_statement(const int &pos, const Symbol &function_symbol) {
    current_ = child(pos);
    Token result;
...
// 解析 read 语句
Token Semantic::analyse_read_statement(const int &pos) {
    current_ = child(pos);
...
// 解析 write 语句
Token Semantic::analyse_write_statement(const int &pos) {
    current_ = child(pos);
...
// 解析赋值语句
Token Semantic::analyse_assign_statement(const int &pos) {
    current_ = child(pos);
...
// 解析定义语句
Token Semantic::analyse_declare_statement(const int &pos) {
    current_ = child(pos);
...
// 解析 return 语句
Token Semantic::analyse_return_statement(const int &pos, const Symbol &function_symbol) {
    current_ = child(pos);
...
// 分析定义关键字
Token Semantic::analyse_declare_keyword(const int &pos) {
    current_ = child(pos);
    Token result;
...
// 解析带数组偏移的标识符
Token Semantic::analyse_identity_array(const int &pos) {
    current_ = child(pos);
    Token result;
...
// 解析判别式
Token Semantic::analyse_condition(const int &pos) {
    current_ = child(pos);
    Token result;
...
// 解析关系运算符
Token Semantic::analyse_comparison_op(const int &pos) {
    current_ = child(pos);
...
// 解析标识符
Token Semantic::analyse_identity(const int &pos) {
...
Token Semantic::analyse_literal(const int &pos) {
    Token result = child(pos)->token();
...
// 解析表达式
Token Semantic::analyse_expression(const int &pos) {
    current_ = child(pos);
    Token result;
...
// 解析算术式
Token Semantic::analyse_term(const int &pos) {
    current_ = child(pos);
    Token result;
...
// 解析元素
Token Semantic::analyse_factor(const int &pos) {
    current_ = child(pos);
    Token result, tmp;
...
Token Semantic::analyse_add_op(const int &pos) {
    return child(pos)->child(0)->token();
}

Token Semantic::analyse_mul_op(const int &pos) {
    return child(pos)->child(0)->token();
}

Token Semantic::analyse_function_call(const int &pos) {
    current_ = child(pos);
...
Token Semantic::analyse_function_call_parameters(const int &pos, const Symbol &function_symbol) {
    current_ = child(pos);
...
void Semantic::print_warning_messages() const {
...
void Semantic::print_ir() const {
    std::cout << ir_ << std::endl;
}

AbstractSyntaxNode *Semantic::child(int offset) {
    return current_->children()[offset];
}

Token::Type Semantic::child_type(int offset) {
    return child(offset)->token().type();
}

int Semantic::children_size() {
    return (int)current_->children().size();
}

Token::Type Semantic::current_token_type() {
    return current_->token().type();
}

void Semantic::add_error_messages(const Position &position, const std::string &message) {
    error_messages_.push_back(std::pair<Position, std::string>(position, message));
}

void Semantic::add_warning_messages(const Position &position, const std::string &message) {
    warning_messages_.push_back(std::pair<Position, std::string>(position, message));
}

void Semantic::build_function_ir_start(const Token &identity) {
...
void Semantic::build_function_ir_end(const Token &identity) {
...
void Semantic::build_assign_statement_ir(const Token &left_identity, const Token &right_expression) {
...
void Semantic::build_expression_ir(const Token &op) {
...
void Semantic::build_term_ir(const Token &op) {
...
void Semantic::build_function_call_ir(const Token &identity) {
    ir_.add(PCode(PCode::Type::kCall, identity.content(), ir_indent_));
}

void Semantic::build_write_statement_ir() {
    ir_.add(PCode(PCode::Type::kPrint, ir_indent_));
}

void Semantic::build_read_statement_ir(const Token &token, const Symbol &symbol) {
...
void Semantic::build_condition_ir(const Token &op) {
    switch (op.type()) {
        case Token::Type::kLT:
...
void Semantic::build_while_statement_ir_end(const std::string &signature) {
...
void Semantic::build_if_statement_ir_else(const std::string &signature) {
